...
#define FT_BITMAP_DUMP_STRUCTURE(pBitmap)          {FT_Bitmap_Pixel_MN pPixelFuncs4Dump = FT_Bitmap_Get_PixelFuncsByBitmap(pBitmap); \
...
#define FT_BITMAP_DUMP_SLOT(slot)                  DEBUG_FORMAT("slot=(%2d, %2d)", slot->bitmap_left, slot->bitmap_top)
#define FT_BITMAP_DUMP(pBitmap)                    FT_Bitmap_Dump(pBitmap, __FUNC__, __LINE__)


/* #define FT_FONT_BITMAP_TO_FILE */


/* static const FT_Bitmap  null_bitmap_ext = { 0, 0, 0, NULL, 0, 0, 0, NULL }; */



  static void FT_Bitmap_Dump(FT_Bitmap*  pBitmap, char* pFunc, int lineNo)
  {
      UCHAR  buffer[8192] = {0};
      int    pos = 0;
      ...
      FT_Bitmap_Pixel_MN* pPixelFuncs = FT_Bitmap_Get_PixelFuncsByBitmap(pBitmap);
  ...
  static FT_Bitmap_Pixel_MN* FT_Bitmap_Get_PixelFuncsByLoadFlags(const int flags, const int gray)
  {
    if (flags & FT_LOAD_TARGET_LCD)
    {
        return NAME_FT_BITMAP_PIXEL_INIT(24, FT_PIXEL_MODE_LCD);
    }
    if (flags & FT_LOAD_TARGET_LCD_V)
    {
        return NAME_FT_BITMAP_PIXEL_INIT(24, FT_PIXEL_MODE_LCD_V);
    }
    if (gray)
    {
        return NAME_FT_BITMAP_PIXEL_INIT( 8, FT_PIXEL_MODE_GRAY);
    }
    return NAME_FT_BITMAP_PIXEL_INIT( 1, FT_PIXEL_MODE_MONO);
  }

  FT_Bitmap_Pixel_MN* FT_Bitmap_Get_PixelFuncsByMode(const int pixel_mode)
  {
      switch (pixel_mode)
      {
  ...
  {
      return FT_Bitmap_Get_PixelFuncsByMode(pBitmap->pixel_mode);
  }

  static FT_Bitmap_Rotate_90N* FT_Bitmap_Get_RotateFuncs(const int degree)
  {
      switch (degree/90)
      {
          case 1:
              return NAME_FT_BITMAP_ROTATE_INIT_90(1);

          case 2:
              return NAME_FT_BITMAP_ROTATE_INIT_90(2);

          case 3:
              return NAME_FT_BITMAP_ROTATE_INIT_90(3);

          case 0:
          default:
              return NAME_FT_BITMAP_ROTATE_INIT_90(0);
      }
  }
  
  static int FT_Bitmap_FloatToInt(float value)
  {
      if (value == 0)
      {
          return 0;
  ...
  int FT_Bitmap_Init_Buffer(FT_Bitmap*  pBitmap)
  {
      ...
      (FT_Bitmap_Get_PixelFuncsByBitmap(pBitmap))->init_pitch(pBitmap);

      pPixelFuncs = FT_Bitmap_Get_PixelFuncsByBitmap(pBitmap);
  ...
  static void FT_Bitmap_FlipL2R(FT_Bitmap* pDstBitmap, const FT_Bitmap* pSrcBitmap,
      FT_GlyphSlot slot)
  {
      ...
      int    x;
      int    y;
      FT_Bitmap_Pixel_MN* pPixelFuncs = FT_Bitmap_Get_PixelFuncsByBitmap(pSrcBitmap);
  ...
  static void FT_Bitmap_FlipT2B(FT_Bitmap* pDstBitmap, const FT_Bitmap* pSrcBitmap,
      FT_GlyphSlot slot)
  {
      UCHAR*  pDst = pDstBitmap->buffer;
      UCHAR*  pSrc = pSrcBitmap->buffer;
   ...
   */
  static UCHAR* FT_Bitmap_Flip(FT_Bitmap* pBitmap, FT_GlyphSlot slot, int flags)
  {
      FT_Bitmap oFliped;
      FT_Bitmap_Pixel_MN* pPixelFuncs = FT_Bitmap_Get_PixelFuncsByBitmap(pBitmap);
      int size;
...
#ifdef FT_FONT_BITMAP_TO_FILE
      FT_BITMAP_DUMP(pBitmap);
#endif
      ...
      if (FT_CHECK_FLIP_L2R(flags))
      {
          ...
          FT_Bitmap_FlipL2R(&oFliped, pBitmap, slot);

          memcpy(pBitmap->buffer, oFliped.buffer, size);
      }

      if (FT_CHECK_FLIP_T2B(flags))
      {
          memset(oFliped.buffer, 0, size);

          FT_Bitmap_FlipT2B(&oFliped, pBitmap, slot);
      }

      FT_Bitmap_Change_Buffer(pBitmap, &oFliped);
  ...
  static void FT_Bitmap_Rotate_Degree(FT_Bitmap* pDstBitmap, const FT_Bitmap* pSrcBitmap,
      const FT_Bitmap_Rotate_90N* pFuncs, const FT_Bitmap_Pixel_MN* pPixelFuncs)
  {
      UCHAR*   pDst = pDstBitmap->buffer;
      UCHAR*   pSrc = pSrcBitmap->buffer;
      int src_width;
  ...
  static UCHAR* FT_Bitmap_Rotate(FT_Bitmap* pBitmap, int degree, FT_GlyphSlot slot, const int flags)
  {
      ...
      pPixelFuncs  = FT_Bitmap_Get_PixelFuncsByBitmap(pBitmap);
      pRotateFuncs = FT_Bitmap_Get_RotateFuncs(degree);
      ...
      FT_Bitmap_Rotate_Degree(&oRotated, pBitmap, pRotateFuncs, pPixelFuncs);
      ...
      FT_Bitmap_Change_Buffer(pBitmap, &oRotated);
...
static int FT_Bitmap_Italic_Hor_Init(FT_Bitmap* pDst,
                                        const float oblique, FT_Bitmap* pSrc, FT_GlyphSlot slot,
                                        FT_Bitmap_Pixel_MN* pPixelFuncs)
{
    int size;
    int rows;
    ...
    italic      = FT_Bitmap_FloatToInt(size*oblique);

    pPixelFuncs->set_width(pDst, pPixelFuncs->get_width(pSrc) + italic);
    pPixelFuncs->set_rows( pDst, pPixelFuncs->get_rows(pSrc));

    FT_Bitmap_Init_Buffer(pDst);

    return italic;
}

static int FT_Bitmap_Italic_Hor_SlotOffset(const int degree, const int to_bottom,
                                              const float oblique, FT_Bitmap* pSrc, FT_GlyphSlot slot,
                                              FT_Bitmap_Pixel_MN* pPixelFuncs)
{
    int size;
    ...
    if (rows > size)
    {
        ...
        slotOffset = FT_Bitmap_FloatToInt(below*oblique);
    }
    else if (degree == 180)
    {
        slotOffset = FT_Bitmap_FloatToInt(size*oblique);
    }
    else
    {
        slotOffset = 0;
 ...
 */
int FT_Bitmap_Italic_Hor(FT_Bitmap* pBitmapDst, const int degree, const int to_bottom, float oblique,
    FT_Bitmap* pBitmapSrc, FT_GlyphSlot slot, const FT_Bitmap_Pixel_MN*   pPixelFuncs)
{
    int isCCW;
    int y;
    int x;
    ...
    totalItalic = FT_Bitmap_Italic_Hor_Init(pBitmapDst, oblique, pBitmapSrc, slot, pPixelFuncs);
    slotOffset  = FT_Bitmap_Italic_Hor_SlotOffset(degree, to_bottom, oblique, pBitmapSrc, slot, pPixelFuncs);
    bufferDst   = pBitmapDst->buffer;
    for ( y = 0; y < rowsSrc; y++ )
    {
        move  = FT_Bitmap_FloatToInt((topSrc - y)*oblique) + slotOffset;
        if (isCCW)
        {
...
static int FT_Bitmap_Italic_Ver_Init(FT_Bitmap* pDst,
                                        const float oblique, FT_Bitmap* pSrc, FT_GlyphSlot slot,
                                        FT_Bitmap_Pixel_MN* pPixelFuncs)
{
    int size;
...
static int FT_Bitmap_Italic_Ver_SlotOffset(const int to_bottom, const int degree,
                                              const float oblique, FT_Bitmap* pSrc, FT_GlyphSlot slot,
                                              FT_Bitmap_Pixel_MN* pPixelFuncs)
{
    int size;
    ...
    {
        slotOffset = FT_Bitmap_FloatToInt(slot->bitmap_left* oblique);
    }
 ...
 */
int FT_Bitmap_Italic_Ver(FT_Bitmap* pDstBitmap, const int degree, const int to_bottom, float oblique,
    FT_Bitmap* pSrcBitmap, FT_GlyphSlot slot, const FT_Bitmap_Pixel_MN*   pPixelFuncs)
{
    int isCCW;
    int y;
    int x;
    ...
    totalItalic = FT_Bitmap_Italic_Ver_Init(pDstBitmap, oblique, pSrcBitmap, slot, pPixelFuncs);
    slotOffset  = FT_Bitmap_Italic_Ver_SlotOffset(to_bottom, degree, oblique, pSrcBitmap, slot, pPixelFuncs);
    bufferDst   = pDstBitmap->buffer;

    for ( x = 0; x < widthSrc; x++ )
    {
        move  = FT_Bitmap_FloatToInt((x+slotLeft)*oblique) - slotOffset;
        if (isCCW)
        {
            move = totalItalic-move;
        }
  ...
  {
      return FT_Bitmap_Italic_Direction(slot,
                 FT_FONT_ITALIC_VALUE,
                 FT_POSTURE_TO_RIGHT,
                 0);
  }
                    ...
                    int         degree)
  {
    ...
    FT_Bitmap_Dump(bitmap);
#endif

    pPixelFuncs  = FT_Bitmap_Get_PixelFuncsByBitmap(bitmap);
    pRotateFuncs = FT_Bitmap_Get_RotateFuncs(degree);

    memcpy(&oItalic, bitmap, sizeof(FT_Bitmap));
    ...
    if (oItalic.buffer != NULL)
    {
        ...
        FT_Bitmap_Change_Buffer(bitmap, &oItalic);
    }
  ...
  static void FT_Bitmap_Create_Buffer_Bit2Byte(FT_Bitmap* pBitmap,
      FT_Int32     load_flags)
  {
      UINT   x, y;
      UCHAR* pSrcData;
      ...
      int    rowsSrc;
      FT_Bitmap oTemp = {0};
      FT_Bitmap_Pixel_MN* pPixelFuncsSrc;
      FT_Bitmap_Pixel_MN* pPixelFuncsDst;

      pixelModeSrc = pBitmap->pixel_mode;
      pPixelFuncsSrc = FT_Bitmap_Get_PixelFuncsByBitmap(pBitmap);

      pPixelFuncsDst = FT_Bitmap_Get_PixelFuncsByLoadFlags(load_flags, 1);
      ...
      FT_Bitmap_Change_Buffer(pBitmap, &oTemp);
  }
   ...
   */
  static void FT_Bitmap_Create_BufferWeight(FT_Bitmap* pBitmap, int xstr, int ystr)
  {
      UCHAR* pSrcData;
      UCHAR* pDstData;
      FT_Bitmap oTemp = {0};
      ...
      memcpy(&oTemp, pBitmap, sizeof(FT_Bitmap));
...
static int
FT_Bitmap_Bold_Action_Default( FT_Bitmap*  pBitmap,
                      FT_Pos      xstr,
                      FT_Pos      ystr )
{
    UCHAR*     p;
...
/**
 ...
 pCurrent[x  ] = 0x00
 */
static void FT_Bitmap_Bold_VerLine(FT_Bitmap*  pBitmap, int width,
      UCHAR* pCurrent, FT_Int  x, FT_Int  y, int pixel_black,
      FT_Bitmap_Pixel_MN* pPixelFuncs)
{
    UCHAR* pMove;
    UCHAR fill;
    ...
    {
        fill = FT_BITMAP_PIXEL_GRAY_DARK;
    }
    else if (height2 > 1 && height == height2+1)
    {
        fill = FT_BITMAP_PIXEL_GRAY_LIGHT+FT_BITMAP_PIXEL_GRAY_HEAVY/height;
    }
    else if (blank != height)
    {
        fill = FT_BITMAP_PIXEL_GRAY_HEAVY;
    }
    else
    {
        fill = FT_BITMAP_PIXEL_GRAY_LIGHT;
    }
 ...
 */
static int
FT_Bitmap_Bold_Action_Gray( FT_Bitmap*  pBitmap,
                      FT_Pos      xstr,
                      FT_Pos      ystr )
{
    UCHAR*     pCurrent;
    ...
    pixel_black = FT_BITMAP_PIXEL_BLACK;
    ...
    for ( y = 0; y < rows; y++ )
    {
        ...
        for ( x = width-1; x > 0; x-- )
        {
            ...
            {
                FT_Bitmap_Bold_VerLine(pBitmap, width, pCurrent, x, y, pixel_black, pPixelFuncs);
            }
        }
    ...
    for ( y = 0; y < rows; y++ )
    {
        for ( x = 0; x < width; x++ )
        {
            if (pCurrent[x] == FT_BITMAP_PIXEL_GRAY_DARK)
            {
                           ...
                           FT_UInt     ypixels )
  {
    ...
    if ( FT_QALLOC_MULT( buffer, height + ypixels, new_pitch ) )
      return error;
                         ...
                         FT_Int*      pYstr)
  {
     ...
     */
    (*pXstr) = (FT_Int)FT_PIX_ROUND( xStrength ) >> 6;
    (*pYstr) = (FT_Int)FT_PIX_ROUND( yStrength ) >> 6;
   ...
   */
  static FT_Error
  FT_Bitmap_Check_Pixel(FT_Library   library,
                         FT_Bitmap*   bitmap,
                         FT_Int*      pXstr,
                         FT_Int*      pYstr)
  {
  ...
  FT_Bitmap_Embolden( FT_Library   library,
                         ...
                         FT_Pos       strength )
  {
    return FT_Bitmap_EmboldenXY(library, bitmap, slot, strength, strength, FT_LOAD_DEFAULT, 0);
  }
                           ...
                           int             office_flags)
  {
    ...
    error = FT_Bitmap_Check_XYStrength(
        xStrength, yStrength,
        &xstr, &ystr);
    if (error)
    {
...
#if 0
    error = FT_Bitmap_Check_Pixel(library, bitmap,
        &xstr, &ystr);
    if (error)
    {
        return error;
    ...
    if (   FT_CHECK_BITMAP_BOLD_GRAY(office_flags)
        || (degree ==   0 && FT_CHECK_BITMAP_BOLD_GRAY_NORTH(office_flags))
        || (degree ==  90 && FT_CHECK_BITMAP_BOLD_GRAY_EAST( office_flags))
        || (degree == 180 && FT_CHECK_BITMAP_BOLD_GRAY_SORTH(office_flags))
        || (degree == 270 && FT_CHECK_BITMAP_BOLD_GRAY_WEST( office_flags))
        )
    {
        FT_Bitmap_Create_Buffer_Bit2Byte(bitmap, load_flags);
        grayFlag = 1;
    }

    if (degree != 0)
    {
        FT_Bitmap_Rotate(bitmap, 360-degree, NULL, office_flags);
    }

    FT_Bitmap_Create_BufferWeight(bitmap, xstr, ystr);

    if (grayFlag)
    {
        FT_Bitmap_Bold_Action_Gray(bitmap, xstr, ystr);
    }
    else
    {
        FT_Bitmap_Bold_Action_Default(bitmap, xstr, ystr);
    }

    if (degree != 0)
    {
        FT_Bitmap_Rotate(bitmap, degree, NULL, office_flags);
    }
                      ...
                      int             office_flags)
  {
    ...
    degree = FT_GlyphSlot_Get_Degree_From_Slot(slot);
    if (!FT_GlyphSlot_Is_Valid_BitmapDegree(degree))
    {
        return FT_Err_Ok;
    }
    ...
    /** process flip first */
    FT_Bitmap_Flip(bitmap, slot, office_flags);

    FT_Bitmap_Rotate(bitmap, degree, slot, office_flags);
