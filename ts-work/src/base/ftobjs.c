 ...
 */
  ...
  FT_Load_Glyph( FT_Face   face,
                 ...
                 FT_Int32  load_flags )
  {
      return FT_Load_Glyph_Office(face, glyph_index, load_flags, 0);
  }
 ...
 */
      ...
      if ( internal->transform_flags
          || FT_CHECK_FLIP_L2R(office_flags)
          || FT_CHECK_FLIP_T2B(office_flags))
      {
        /* get renderer */
        FT_Renderer  renderer = ft_lookup_glyph_renderer( slot );
        int degree = FT_GlyphSlot_Get_Degree_From_Slot(slot);
        /* if bitmap and rotate right degrees(like 90, 180, 270), use FT_GlyphSlot_Transform to do it. */
        if ( renderer
#if defined(FT_BITMAP_RIGHT_ANGLE_ENABLE)
             && (slot->format != FT_GLYPH_FORMAT_BITMAP || !FT_GlyphSlot_Is_Valid_BitmapDegree(degree))
#endif
            )
        {
          FT_Matrix  transform_matrix = {0};
          
          if (FT_CHECK_FLIP_L2R(office_flags))
          {
              transform_matrix.xx = -0x10000L;
              transform_matrix.yx =  0;
              transform_matrix.xy =  0;
              transform_matrix.yy =  0x10000L;
              FT_Outline_Transform( &slot->outline, &transform_matrix );
          }

          if (FT_CHECK_FLIP_T2B(office_flags))
          {
              transform_matrix.xx =  0x10000L;
              transform_matrix.yx =  0;
              transform_matrix.xy =  0;
        ...
        {
            FT_Bitmap_Load_Glyph(slot->library, &slot->bitmap, slot, load_flags, office_flags);
        }
